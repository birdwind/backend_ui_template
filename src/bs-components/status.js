...
const template = `
    <div v-if="getOption(status)">
      ...
      <el-form
          ...
          label-width="70px"
      >
        <el-form-item :label="$t('system.ui.template.status')" prop="newStatus">
          <bs-select v-model="newStatus" :options="selectableOptions" :clearable="false" />
        </el-form-item>
        <el-form-item :label="$t('system.ui.template.memo')">
...
const props = {
  ...
  title: {
    default: i18n.t('system.ui.template.changeStatus'),
    type: String
  }
}
...
const methods = {
  ...
  onSubmit() {
    ...
    if (this.newStatus === null || this.newStatus === undefined || this.newStatus === '') {
      Message({
        message: i18n.t('status.allStatus') + i18n.t('system.ui.template.status'),
        type: 'error',
        duration: 5 * 1000
      })
      return
